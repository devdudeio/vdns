...
echo "Testing ${VNS_TEST_HOST} local redirect path"
echo
...
HOST_HEADER_OUTPUT="$(curl -i --max-time 5 -H "Host: ${VNS_TEST_HOST}" "http://127.0.0.1:${VNS_REDIRECT_PORT}/" 2>&1)"
if printf '%s\n' "${HOST_HEADER_OUTPUT}" | grep -Eiq '^HTTP/[^ ]+ 302 ' &&
  printf '%s\n' "${HOST_HEADER_OUTPUT}" | grep -Eiq '^location: http://chainvue\.io/\r?$'; then
  pass "redirect service on port ${VNS_REDIRECT_PORT} returned chainvue redirect"
else
  fail "redirect service on port ${VNS_REDIRECT_PORT} did not return expected redirect"
...
if printf '%s\n' "${PORT80_OUTPUT}" | grep -Eiq '^HTTP/[^ ]+ 302 ' &&
  printf '%s\n' "${PORT80_OUTPUT}" | grep -Eiq '^location: http://chainvue\.io/\r?$'; then
  pass "http://${VNS_TEST_HOST} returned chainvue redirect"
else
  fail "http://${VNS_TEST_HOST} did not return expected redirect"
  echo "${PORT80_OUTPUT}"
fi

echo
if [[ "${FAILED}" -eq 0 ]]; then
  echo "Success: http://${VNS_TEST_HOST} is resolving locally and redirecting."
  exit 0
fi
...
if [[ "${RESOLVER_OK}" -ne 1 ]]; then
  ...
  echo "Next step: redirect service is not listening on port ${VNS_REDIRECT_PORT}. Start it with: VNS_RESOLVER_URL=${VNS_RESOLVER_URL} VNS_REDIRECT_PORT=${VNS_REDIRECT_PORT} pnpm redirect:dev"
elif [[ "${REDIRECT_80_LISTENING}" -ne 1 ]]; then
  echo "Next step: redirect service is not listening on port 80. Start it with: pnpm build && sudo scripts/macos/start-redirect-port80.sh"
elif printf '%s\n' "${HOST_HEADER_OUTPUT}" | grep -Eiq '^location: http://chainvue\.io/\r?$' &&
  ! printf '%s\n' "${PORT80_OUTPUT}" | grep -Eiq '^location: http://chainvue\.io/\r?$'; then
  echo "Next step: port ${VNS_REDIRECT_PORT} Host-header redirect works, but http://${VNS_TEST_HOST} does not. Start the redirect service on port 80."
  echo "Run: pnpm build && sudo scripts/macos/start-redirect-port80.sh"
else
